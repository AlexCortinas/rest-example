...
public class DatabaseLoader {
  ...
  public void loadData() throws UserLoginExistsException {
    ...
    Post post = new Post("Título 1", "Texto del primer post",
        userDAO.findByLogin("pepe"));
    post.getTags().add(news);
    post.getTags().add(podcast);
    postDAO.save(post);
    post = new Post("Título 2", "Texto del segundo post",
        userDAO.findByLogin("maria"));
    post.getTags().add(news);
    post.getTags().add(tech);
    postDAO.save(post);
    post = new Post("Título 3", "Texto del tercero post",
        userDAO.findByLogin("maria"));
    postDAO.save(post);
    post = new Post("Título 4", "Texto del cuarto post",
        userDAO.findByLogin("maria"));
    postDAO.save(post);
    post = new Post("Título 5", "Texto del quinto post",
        userDAO.findByLogin("pepe"));
    postDAO.save(post);
    post = new Post("Título 6", "Texto del sexto post",
        userDAO.findByLogin("pepe"));
    postDAO.save(post);
  }
}
